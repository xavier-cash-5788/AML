(function (global, factory) {
  ...
  function getMountedOrPendingDependents(atom, atomState, mountedMap) {
    ...
    var mountedDependents = mounted == null ? void 0 : mounted.t;
    var pendingDependents = atomState.p;
    if (!(mountedDependents != null && mountedDependents.size)) {
  ...
  var BUILDING_BLOCK_ensureAtomState = function BUILDING_BLOCK_ensureAtomState(buildingBlocks, store, atom) {
    ...
    if (!atomState) {
      var _storeHooks = buildingBlocks[6];
      var _atomOnInit = buildingBlocks[9];
      atomState = {
        d: new Map(),
        p: new Set(),
        n: 0
      ...
      if (hasOnInit(atom)) {
        _atomOnInit(buildingBlocks, store, atom);
      }
    }
    return atomState;
  };
  ...
  var BUILDING_BLOCK_readAtomState = function BUILDING_BLOCK_readAtomState(buildingBlocks, store, atom) {
    ...
    var storeEpochHolder = buildingBlocks[28];
    ...
    if (isAtomStateInitialized(atomState)) {
      ...
      for (var _iterator0 = _createForOfIteratorHelperLoose(atomState.d), _step0; !(_step0 = _iterator0()).done;) {
        var _step0$value = _step0.value,
          a = _step0$value[0],
          _n = _step0$value[1];
        if (readAtomState(buildingBlocks, store, a).n !== _n) {
          hasChangedDeps = true;
          break;
        }
      }
    ...
    var getter = function getter(a) {
      if (a === atom) {
        var _aState2 = ensureAtomState(buildingBlocks, store, a);
      ...
      var aState = readAtomState(buildingBlocks, store, a);
      try {
  ...
  var BUILDING_BLOCK_writeAtomState = function BUILDING_BLOCK_writeAtomState(buildingBlocks, store, atom, args) {
    ...
    var isSync = true;
    var getter = function getter(a) {
      return returnAtomValue(readAtomState(buildingBlocks, store, a));
    };
    ...
    if (!mounted) {
      ...
      if (isActuallyWritableAtom(atom) && hasOnMount(atom)) {
        var processOnMount = function processOnMount() {
          ...
          try {
            var onUnmount = atomOnMount(buildingBlocks, store, atom, setAtom);
            if (onUnmount) {
              mounted.u = function () {
                isSync = true;
                try {
  ...
  var BUILDING_BLOCK_storeSub = function BUILDING_BLOCK_storeSub(buildingBlocks, store, atom, listener) {
    ...
    var mountAtom = buildingBlocks[18];
    var unmountAtom = buildingBlocks[19];
    var mounted = mountAtom(buildingBlocks, store, atom);
  ...
  function buildStore() {
    ...
    var storeGet = buildingBlocks[21];
    var storeSet = buildingBlocks[22];
    var storeSub = buildingBlocks[23];
    return store;
  }
